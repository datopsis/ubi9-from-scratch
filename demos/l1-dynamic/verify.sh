...
# it computes it.
#
# Usage: demos/l1-dynamic/verify.sh <image>
...
echo "$report" | sed 's/^/  /'
# The point of this rung is the opposite of L0: libraries ARE loaded from the
# image, and the report must show them.
if echo "$report" | grep -q 'shared libraries     : 0'; then
    echo "  FAIL  expected shared libraries to be mapped — this is a dynamic build"
    failures=$((failures + 1))
else
    echo "  PASS  shared libraries are loaded from the image"
fi
